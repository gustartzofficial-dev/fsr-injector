...
- Genuine AMD FidelityFX FSR 1.0 on both backends: EASU + RCAS with render scaling on DX12, and RCAS sharpening on DX11, with automatic fallback to the previous approximation shaders if the FSR shaders fail to compile at runtime (the log states which path is active).
